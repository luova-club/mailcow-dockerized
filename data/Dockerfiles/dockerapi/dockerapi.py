        ...
        try:
            for container in await async_docker_client.containers.list():
                if container._id == container_id:
                    ...
                    return Response(
                        content=json.dumps(container_info, indent=4),
                        media_type="application/json",
                    )

            res = {"type": "danger", "msg": "no container found"}
            return Response(
                content=json.dumps(res, indent=4), media_type="application/json"
            )
        except Exception as e:
            res = {"type": "danger", "msg": str(e)}
            return Response(
                content=json.dumps(res, indent=4), media_type="application/json"
            )
    else:
        res = {"type": "danger", "msg": "no or invalid id defined"}
        return Response(
            content=json.dumps(res, indent=4), media_type="application/json"
        )
...
async def get_containers():
    ...
    try:
        ...
        return Response(
            content=json.dumps(containers, indent=4), media_type="application/json"
        )
    except Exception as e:
        res = {"type": "danger", "msg": str(e)}
        return Response(
...
async def post_containers(container_id: str, post_action: str, request: Request):
    ...
    if container_id and container_id.isalnum() and post_action:
        try:
            ...
            if post_action == "exec":
                if not request_json or not "cmd" in request_json:
                    res = {"type": "danger", "msg": "cmd is missing"}
                    return Response(
                        content=json.dumps(res, indent=4), media_type="application/json"
                    )
                if not request_json or not "task" in request_json:
                    res = {"type": "danger", "msg": "task is missing"}
                    return Response(
                        content=json.dumps(res, indent=4), media_type="application/json"
                    )

                api_call_method_name = "__".join(
                    [
                        "container_post",
                        str(post_action),
                        str(request_json["cmd"]),
                        str(request_json["task"]),
                    ]
                )
            else:
            ...
            api_call_method = getattr(
                docker_utils,
                api_call_method_name,
                lambda container_id: Response(
                    content=json.dumps(
                        {"type": "danger", "msg": "container_post - unknown api call"},
                        indent=4,
                    ),
                    media_type="application/json",
                ),
            ...
            print(
                "api call: %s, container_id: %s" % (api_call_method_name, container_id)
            )
        ...
        res = {"type": "danger", "msg": "invalid container id or missing action"}
        return Response(
            content=json.dumps(res, indent=4), media_type="application/json"
        )
...
class DockerUtils:
        ...
        for container in await self.docker_client.containers.list():
            if container._id == container_id:
                ...
                if exec_details["ExitCode"] == None or exec_details["ExitCode"] == 0:
                    res = {"type": "success", "msg": ps_return.data.decode("utf-8")}
                    return Response(
                        content=json.dumps(res, indent=4), media_type="application/json"
                    )
                else:
    ...
    async def container_post__exec__mailq__delete(self, container_id, request_json):
        if "items" in request_json:
            ...
            if filtered_qids:
                ...
                for container in await self.docker_client.containers.list():
                    if container._id == container_id:
                        postsuper_r_exec = await container.exec(
                            [
                                "/bin/bash",
                                "-c",
                                "/usr/sbin/postsuper " + sanitized_string,
                            ]
                        )
    ...
    async def container_post__exec__mailq__cat(self, container_id, request_json):
        if "items" in request_json:
            ...
            if filtered_qids:
                ...
                for container in await self.docker_client.containers.list():
                    if container._id == container_id:
                        postcat_exec = await container.exec(
                            [
                                "/bin/bash",
                                "-c",
                                "/usr/sbin/postcat -q " + sanitized_string,
                            ],
                            user="postfix",
                        )
                ...
                for container in await self.docker_client.containers.list():
                    if container._id == container_id:
                        for i in flagged_qids:
                            postsuper_r_exec = await container.exec(
                                ["/bin/bash", "-c", "/usr/sbin/postqueue " + i],
                                user="postfix",
                            )
    ...
    async def container_post__exec__mailq__list(self, container_id, request_json):
        for container in await self.docker_client.containers.list():
            if container._id == container_id:
                mailq_exec = await container.exec(
                    ["/usr/sbin/postqueue", "-j"], user="postfix"
                )
    ...
    async def container_post__exec__mailq__flush(self, container_id, request_json):
        for container in await self.docker_client.containers.list():
            if container._id == container_id:
                postsuper_r_exec = await container.exec(
                    ["/usr/sbin/postqueue", "-f"], user="postfix"
                )
                return await exec_run_handler("generic", postsuper_r_exec)

    # api call: container_post - post_action: exec - cmd: mailq - task: super_delete
    async def container_post__exec__mailq__super_delete(
        self, container_id, request_json
    ):
        for container in await self.docker_client.containers.list():
            if container._id == container_id:
                postsuper_r_exec = await container.exec(
    ...
    async def container_post__exec__system__fts_rescan(
        self, container_id, request_json
    ):
        if "username" in request_json:
            for container in await self.docker_client.containers.list():
                if container._id == container_id:
                    rescan_exec = await container.exec(
                        [
                            "/bin/bash",
                            "-c",
                            "/usr/bin/doveadm fts rescan -u '"
                            + request_json["username"].replace("'", "'\\''")
                            + "'",
                        ],
                        user="vmail",
                    )
                    ...
                    if (
                        exec_details["ExitCode"] == None
                        or exec_details["ExitCode"] == 0
                    ):
        ...
        if "all" in request_json:
            for container in await self.docker_client.containers.list():
                if container._id == container_id:
                    rescan_exec = await container.exec(
                        ["/bin/bash", "-c", "/usr/bin/doveadm fts rescan -A"],
                        user="vmail",
                    )
                    async with rescan_exec.start(detach=False) as stream:
                        rescan_return = await stream.read_out()
    ...
    async def container_post__exec__system__df(self, container_id, request_json):
        if "dir" in request_json:
            for container in await self.docker_client.containers.list():
                if container._id == container_id:
                    df_exec = await container.exec(
                        [
                            "/bin/bash",
                            "-c",
                            "/bin/df -H '"
                            + request_json["dir"].replace("'", "'\\''")
    ...
    async def container_post__exec__system__mysql_upgrade(
        self, container_id, request_json
    ):
        for container in await self.docker_client.containers.list():
            if container._id == container_id:
                sql_exec = await container.exec(
                    [
                        "/bin/bash",
                        "-c",
                        "/usr/bin/mysql_upgrade -uroot -p'"
                        + os.environ["DBROOT"].replace("'", "'\\''")
                        + "'\n",
                    ],
                    user="mysql",
                )
    ...
    async def container_post__exec__system__mysql_tzinfo_to_sql(
        self, container_id, request_json
    ):
        for container in await self.docker_client.containers.list():
            if container._id == container_id:
                sql_exec = await container.exec(
                    [
                        "/bin/bash",
                        "-c",
                        "/usr/bin/mysql_tzinfo_to_sql /usr/share/zoneinfo | /bin/sed 's/Local time zone must be set--see zic manual page/FCTY/' | /usr/bin/mysql -uroot -p'"
                        + os.environ["DBROOT"].replace("'", "'\\''")
                        + "' mysql \n",
                    ],
                    user="mysql",
    ...
    async def container_post__exec__reload__dovecot(self, container_id, request_json):
        for container in await self.docker_client.containers.list():
            if container._id == container_id:
                reload_exec = await container.exec(
                    ["/bin/bash", "-c", "/usr/sbin/dovecot reload"]
                )
    ...
    async def container_post__exec__reload__postfix(self, container_id, request_json):
        for container in await self.docker_client.containers.list():
            if container._id == container_id:
                reload_exec = await container.exec(
                    ["/bin/bash", "-c", "/usr/sbin/postfix reload"]
                )
                return await exec_run_handler("generic", reload_exec)
    ...
    async def container_post__exec__reload__nginx(self, container_id, request_json):
        for container in await self.docker_client.containers.list():
            if container._id == container_id:
                reload_exec = await container.exec(
                    ["/bin/sh", "-c", "/usr/sbin/nginx -s reload"]
                )
                return await exec_run_handler("generic", reload_exec)
    ...
    async def container_post__exec__sieve__list(self, container_id, request_json):
        if "username" in request_json:
            for container in await self.docker_client.containers.list():
                if container._id == container_id:
                    sieve_exec = await container.exec(
                        [
                            "/bin/bash",
                            "-c",
                            "/usr/bin/doveadm sieve list -u '"
                            + request_json["username"].replace("'", "'\\''")
                            + "'",
                        ]
                    )
            ...
            for container in await self.docker_client.containers.list():
                if container._id == container_id:
                    cmd = [
                        "/bin/bash",
                        "-c",
                        "/usr/bin/doveadm sieve get -u '"
                        + request_json["username"].replace("'", "'\\''")
                        + "' '"
                        + request_json["script_name"].replace("'", "'\\''")
                        + "'",
                    ]
            ...
            for container in await self.docker_client.containers.list():
                if container._id == container_id:
                    ...
                    cmd = [
                        ...
                        "-c",
                        "if [[ -d '/var/vmail/"
                        + request_json["maildir"].replace("'", "'\\''")
                        + "' ]]; then /bin/mv '/var/vmail/"
                        + request_json["maildir"].replace("'", "'\\''")
                        + "' '/var/vmail/_garbage/"
                        + str(int(time.time()))
                        + "_"
    ...
    async def container_post__exec__rspamd__worker_password(
        self, container_id, request_json
    ):
...
async def exec_run_handler(type, exec_obj):
    ...
    if type == "generic":
        ...
        if exec_details["ExitCode"] == None or exec_details["ExitCode"] == 0:
            ...
            res = {"type": "success", "msg": "'command failed: " + exec_return}
            return Response(
                content=json.dumps(res, indent=4), media_type="application/json"
            )
...
if os.environ["REDIS_SLAVEOF_IP"] != "":
    redis_client = redis.Redis(
        host=os.environ["REDIS_SLAVEOF_IP"], port=os.environ["REDIS_SLAVEOF_PORT"], db=0
    )
else:
