...
while True:
    try:
        ...
        if "".__eq__(redis_slaveof_ip):
            r = redis.StrictRedis(
                host=os.getenv("IPV4_NETWORK", "172.22.1") + ".249",
                ...
                for chain in [forward_chain, input_chain]:
                    ...
                    for position, item in enumerate(chain.rules):
                        if item.target.name == "MAILCOW":
                            ...
                            if position > 2:
                                logCrit(
                                    "Error in %s chain order: MAILCOW on position %d, restarting container"
                                    % (chain.name, position)
                                )
                                quit_now = True
                                exit_code = 2
                    if not target_found:
                        logCrit(
                            "Error in %s chain: MAILCOW target not found, restarting container"
                            % (chain.name)
                        )
                        quit_now = True
                        exit_code = 2
...
def ban(address):
    ...
    if bans[net]["attempts"] >= MAX_ATTEMPTS:
        ...
        logWarn(
            "%d more attempts in the next %d seconds until %s is banned"
            ...
            try:
                ...
                for position, rule in enumerate(chain.rules):
                    ...
                    if position == 0:
                        if not match:
                            logInfo(
                                f"Added POSTROUTING rule for source network {new_rule.src} to SNAT target {snat_target}"
                            )
                            chain.insert_rule(new_rule)
                    else:
                        if match:
                            logInfo(
                                f"Remove rule for source network {new_rule.src} to SNAT target {snat_target} from POSTROUTING chain at position {position}"
                            )
                            chain.delete_rule(rule)
                table.commit()
        ...
        with lock:
            try:
                ...
                if get_snat6_rule() not in chain.rules:
                    logInfo(
                        "Added POSTROUTING rule for source network %s to SNAT target %s"
                        % (get_snat6_rule().src, snat_target)
                    )
...
def initChain():
    ...
    # IPv4
    if (
        not iptc.Chain(iptc.Table(iptc.Table.FILTER), "MAILCOW")
        in iptc.Table(iptc.Table.FILTER).chains
    ):
    ...
    # IPv6
    if (
        not iptc.Chain(iptc.Table6(iptc.Table6.FILTER), "MAILCOW")
        in iptc.Table6(iptc.Table6.FILTER).chains
    ):
