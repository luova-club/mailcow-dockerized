...
try:
    ...
    def notify_rcpt(rcpt, msg_count, quarantine_acl, category):
        ...
        meta_query = query_mysql(
            'SELECT SHA2(CONCAT(id, qid), 256) AS qhash, id, subject, score, sender, created, action FROM quarantine WHERE notified = 0 AND rcpt = "%s" AND score < %f AND (action = "%s" OR "all" = "%s")'
            % (rcpt, max_score, category, category)
        )
        print(
            "%s: %d of %d messages qualify for notification"
            % (rcpt, len(meta_query), msg_count)
        )
        if len(meta_query) == 0:
            return
        ...
        while count < 15:
            ...
            try:
                ...
                for res in meta_query:
                    query_mysql(
                        'UPDATE quarantine SET notified = 1 WHERE id = "%d"'
                        % (res["id"]),
                        update=True,
                    )
    ...
    records = query_mysql(
        "SELECT IFNULL(user_acl.quarantine, 0) AS quarantine_acl, count(id) AS counter, rcpt FROM quarantine LEFT OUTER JOIN user_acl ON user_acl.username = rcpt WHERE notified = 0 AND score < %f AND rcpt in (SELECT username FROM mailbox) GROUP BY rcpt"
        % (max_score)
    )
    ...
    for record in records:
        ...
        try:
            ...
            print(
                "Could not determine last notification for %s, assuming never"
                % (record["rcpt"])
            )
            last_notification = 0
        attrs_json = query_mysql(
            'SELECT attributes FROM mailbox WHERE username = "%s"' % (record["rcpt"])
        )
        ...
        if (
            last_notification == 0
            or (last_notification + time_trans[attrs["quarantine_notification"]])
            <= time_now
        ):
            print(
            ...
            notify_rcpt(
                record["rcpt"],
                record["counter"],
                record["quarantine_acl"],
                attrs["quarantine_category"],
            )
