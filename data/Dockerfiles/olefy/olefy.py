...
logging.basicConfig(
    stream=sys.stdout,
    level=olefy_loglvl,
    format="olefy %(levelname)s %(funcName)s %(message)s",
)

logger.debug(
    "olefy listen address string: {} (type {})".format(
...
if not olefy_listen_addr_string:
    ...
    addr_re = re.compile('[\[" \]]')
    olefy_listen_addr = addr_re.sub(
        "", olefy_listen_addr_string.replace("'", "")
    ).split(",")

# log runtime variables
logger.info(
    "olefy listen address: {} (type: {})".format(
        olefy_listen_addr, type(olefy_listen_addr)
    )
)
...
def oletools(stream, tmp_file_name, lid):
    if olefy_min_length > stream.__len__():
        logger.error(
            "{} {} bytes (Not Scanning! File smaller than {!r})".format(
                lid, stream.__len__(), olefy_min_length
            )
        )
        ...
        if out.__len__() < 30:
            logger.error(
                "{} olevba returned <30 chars - rc: {!r}, response: {!r}, error: {!r}".format(
                    lid,
                    cmd_tmp.returncode,
                    out.decode("utf-8", "ignore"),
            ...
            logger.error(
                "{} olevba stderr >10 chars - rc: {!r}, response: {!r}".format(
                    lid, cmd_tmp.returncode, err.decode("utf-8", "ignore")
                )
            )
            ...
            logger.error(
                "{} olevba exited with code {!r}; err: {!r}".format(
                    lid, cmd_tmp.returncode, err.decode("utf-8", "ignore")
                )
            )
            failed = True
        ...
        if failed and olefy_del_tmp_failed == 0:
            logger.debug(
                "{} {} FAILED: not deleting tmp file".format(lid, tmp_file_name)
            )
